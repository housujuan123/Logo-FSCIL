...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        #clipcache = torch.load("/home/sdnu2022/桌面/zjx_code/Logo_FSCIL/Logo2k_mini/mywork/model_minilogo_vitb16.pkl")  #8.12对比注释
        #self.cmodel.load_state_dict(clipcache.state_dict()) 
        
        print("-----Text features loading...")
        self.text_features_matrix = torch.zeros(130, 512).to(self.device)  #text features
        for i in range(130):
    ...
    def test_intergrate(self, model, testloader, epoch, args, session, best_head_dict=None, ocrextracts=None, validation=True):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(testloader, 1):
                ...
                clipscore = F.cosine_similarity(base_features_matrix.unsqueeze(1), self.text_features_matrix.unsqueeze(0), dim=2)   #100*130
                ...
                logits4all = torch.zeros(batchlen, 230)
                ...
                logits_all = logits + logits2 + 0.45*logits3 + 0.15*logits4     #5*logits3 
                ...
                if session > 0:
                    ...
                    acc123 = count_acc(logits+logits2+0.45*logits3, test_label)
                    acc124 = count_acc(logits+logits2+0.15*logits4, test_label)
